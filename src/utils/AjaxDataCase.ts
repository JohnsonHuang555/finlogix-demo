...
export function toSnakeCaseObject<T = AjaxData>(data: AjaxData): T {
  return _.entries(data).reduce(
    (acc, [key, value]) => ({
      ...acc,
...
export function nestedToSnakeCase<T = AjaxData>(data: unknown | unknown[]): T {
  let returnValue: unknown = data;
  if (Array.isArray(data)) {
    returnValue = data.map(nestedToSnakeCase);
  } else if (typeof data === 'object' && data !== null) {
    returnValue = _.entries(data).reduce<AjaxData>((obj, [key, value]) => {
      const snakeKey = _.snakeCase(key);
      obj[snakeKey] = nestedToSnakeCase(value);
      return obj;
    }, {});
  }
  return returnValue as T;
